...
function EntitiesPage() {
  ...
  const handleEntityNameChange = (entityId: string, newName: string) => {
    const updatedEntities = entities.map(e => e.id === entityId ? { ...e, name: newName } : e);
  ...
  const handleAttributeChange = (attrIndex: number, field: 'name' | 'type', value: string) => {
    if (!selectedEntity) return;
  ...
  const handleRemoveAttribute = (attrId: string) => {
    if (!selectedEntity) return;
    const updatedAttributes = selectedEntity.attributes.filter(a => a.id !== attrId);
    const updatedEntity = { ...selectedEntity, attributes: updatedAttributes };
    setSelectedEntity(updatedEntity);
    setEntities(entities.map(e => e.id === selectedEntity.id ? updatedEntity : e));
  };
      ...
      <SidebarInset>
        ...
        <main className="flex-1 space-y-8 p-4 sm:p-6 lg:p-8">
          <div className="flex items-center justify-between space-y-2">
            <div>
                <h1 className="text-3xl font-headline font-bold tracking-tight">Entity Management</h1>
                <p className="text-muted-foreground">Define and manage your application's data structures.</p>
            </div>
            <Button onClick={handleAddNewEntity}><Plus className="mr-2 h-4 w-4" /> Add Entity</Button>
          </div>
            ...
            <div className="md:col-span-3">
              {selectedEntity ? (
                <Card>
                  <CardHeader>
                    <div className='flex items-center gap-4'>
                        <Label htmlFor='entity-name' className='text-lg'>Entity Name:</Label>
                        <Input
                            id='entity-name'
                            className="text-2xl font-semibold leading-none tracking-tight w-1/2"
                            value={selectedEntity.name}
                            onChange={(e) => handleEntityNameChange(selectedEntity.id, e.target.value)}
                        />
                    </div>
                    <CardDescription>Manage the attributes for the "{selectedEntity.name}" entity.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {selectedEntity.attributes.map((attr, index) => (
                      <div key={attr.id} className="grid grid-cols-10 gap-2 items-center">
                        <div className="col-span-4">
                            <Label htmlFor={`attr-name-${attr.id}`}>Attribute Name</Label>
                            <Input
                        ...
                        <div className="col-span-4">
                            <Label htmlFor={`attr-type-${attr.id}`}>Attribute Type</Label>
                             <Select
                                value={attr.type}
                                onValueChange={(value: string) => handleAttributeChange(index, 'type', value)}
                            >
                        ...
                        <div className="col-span-2 flex items-end h-full">
                             <Button variant="ghost" size="icon" onClick={() => handleRemoveAttribute(attr.id)} aria-label="Remove attribute">
                ...
                <div className="flex h-full items-center justify-center rounded-lg border-2 border-dashed border-muted-foreground/20 bg-muted/20 py-32">
                  <p className="text-muted-foreground">Select an entity to view its details or add a new one.</p>
                </div>
              )}
            </div>
          </div>
